...
class StaticFileRoute(object):
    ...
    def __call__(self, *args):
        ...
        ctx.response.content_type = mimetypes.types_map.get(fext.lower(), 'appliction/octet-stream')
...
class Request(object):
    ...
    def __getitem__(self, key):
        '''
        Get input parameter value. If the specified key has multiple valuem the first one is returned.
    ...
    def header(self, header, default = None):
        '''
        Get header from request as unicode, return None if not exist, or default if specfied.
    ...
    def cookies(self):
        '''
        Return all cookies as dict. The cookie name is str and values is nuicode.
...
class Response(object):
    ...
    def content_type(self):
        '''
        Get content type form response. This is a shortcut for header('Content-Type').
...
def _default_error_handler(e, start_response, is_debug):
    ...
    statrt_response('500 Internal Server Error', [('Content-Type', 'text/html'), _HEADER_X_POWERED_BY])
    if is_debug:
        return _debug()
    return ('<html><body><h1>500 Internal Server Error</h1><h3>%3</h3></body></html>' % str(e))
...
def _build_pattern_fn(pattern):
    ...
    m = _RE_INTERCEPTROR_ENDS_WITH.match(patter)
    if m:
