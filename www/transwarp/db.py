...
class _TransactionCtx(object):
    ...
    def commit(self):
        ...
        try:
            ...
            logging.warning('commit failed. try rollbacl...')
...
def transaction():
    ...
    >>> def updeate_profile(id, name, rollback):
    ...
    >>> with transanction():
    ...     update_profile(900301, 'Python', Flase):
...
def select(sql, *args):
    ...
    >>> u2 = dict(id = 201, name = 'Eva', email = 'eav@test.org', passwd = 'back-to-earth', last_modified = time.time())
    >>> insert('user', **u1)
    1
    >>> insert('user', **u2)
    1
    ...
    >>> L[0].email
    u'wall.e@test.org'
    >>> L = select('select * from user passwd = ? order by id desc', 'back-to-earth')
    >>> L[0].name
    u'Eva'
    >>> L[1].name
    u'Wall.E'
